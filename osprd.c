...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
  ...
  if (req->sector < 0 || req->sector > nsectors)
    {
      eprintk("Invalid sector: %lu", (unsigned long)req->sector);
      end_request(req, 0);
    }
  ...
  // check to make sure requested number of sectors do not exceed
  ...
  if (req->sector + req->current_nr_sectors <= nsectors)
  ...
  else
    {
      ...
      eprintk("%lu sectors unavailable. Proceeded with %lu sectors.\n", (unsigned long)req->current_nr_sectors, (unsigned long)(nsectors - req->sector));
    }
